...
dat <- read.delim("PerseusOutputTable_SILAC_EM.txt") # already a dataframe
...
library(readxl)
df_met <- data.frame(read_xlsx("Tables_1.xlsx", sheet = 2))
met <- data.frame(df_met$Gene.Name) # 22

# add some genes with fold changes more than 6 or less than -6
dat_met <- data.frame(dat[which(abs(dat[, 2]) >= 6), 1]) # 8
...
volcano.plot <- function(dat, 
                         ...
                         xintercept = 1, 
                         yintercept = 1.3,
                         col = c("grey","steelblue","red","darkgrey"),
                         title = "",
                         labNames = "D492/D492M",
                         height = 10,
                         width = 18) {
  ...
  if (TRUE %in% grepl("label", colnames(dat), ignore.case = F)) {
    ...
    file <- paste(format(Sys.time(), "%F %H-%M-%S"), "SILAC_EMT_VolcanoPlot.tiff")
